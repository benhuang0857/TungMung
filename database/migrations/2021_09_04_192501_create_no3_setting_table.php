...
class CreateNo3SettingTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('no3_setting', function (Blueprint $table) {
            $table->increments('id');
            // $table->float('C1')->comment('估算後濃度(結果)');
            $table->float('C0')->comment('估算前濃度');
            $table->float('NO3_auto_para')->comment('硝酸自動添加參數');
            ...
            // $table->float('N')->comment('硝酸自動添加參數×線速(M/m)×板寬(mm)×添加時間(5min)×k1');
            // $table->float('W')->comment('H2O自動添加參數×線速(M/m)×板寬(mm)×添加時間(5min)×k2');
            // $table->float('F')->comment('氫氟酸自動添加參數×線速(M/m)×板寬(mm)×添加時間(5min)×k3');
    ...
    {
        Schema::dropIfExists('no3_setting');
    }
}
